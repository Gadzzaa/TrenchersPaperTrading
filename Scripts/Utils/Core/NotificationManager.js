...
export class NotificationManager {
    ...
    async build() {
        try {
            ...
            let fullMessage =
                `${this.#typeClasses[this.type]} ${sanitizedMessage}`;
            ...
            const volume = await StorageManager.getFromStorage("volume");
            this.volume = volume ?? 1.0;

            const sound = this.sound
                ? NotificationHelper.getSound(
                    this.type,
                    this.volume,
                    this.sounds,
                )
            ...
            await NotificationHelper.execNotification(fullMessage, sound);
        } catch (error) {
            ErrorHandler.log(
                new AppError("Failed to show notification", {cause: error}),
            );
        }
    }
}
